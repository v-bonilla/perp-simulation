...
@pytest.fixture
def ohlcv_historical_feather_repository() -> OHLCVRepository:
...
def test_get_historical_data_ohlcv_1min_five_bars(
    ohlcv_historical_feather_repository: OHLCVRepository,
):
    """Get historical OHLCV, 1min, five bars of data.

    Test data is closed left and open right (last datapoint is 2024-01-22 23:59:00).
    """
    symbol = Symbol.BTCUSD
    timeframe = Timeframe.ONE_MIN
    start_date = datetime(2024, 1, 22, 23, 55, tzinfo=timezone.utc)
    iterator = ohlcv_historical_feather_repository.get_historical_data(
